...
export default function ModelSwitcher({ sidebarMode = false }: { sidebarMode?: boolean }) {
  ...
  const getCurrentModelDisplay = () => {
    ...
    return model?.displayName ?? model?.name?.split(":")[0] ?? currentModel?.split(":")[0] ?? "Select model";
  };

  const formatModelName = (model: Model) => model.displayName ?? model.name?.split(":")[0] ?? model.name;
  const formatModelSize = (size?: number) => (size ? `${(size / (1024 * 1024 * 1024)).toFixed(1)} GB` : "");
    ...
    <div className={`relative ${sidebarMode ? "w-full" : ""}`} ref={dropdownRef}>
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {gpuStatus === "connected" && (
            <span className="relative flex h-2 w-2" title="GPU Connected">
      ...
      {isOpen && (
        <div
          className={`absolute ${sidebarMode ? "bottom-full mb-2 left-0 w-full" : "top-full mt-2 left-1/2 -translate-x-1/2 w-72"} py-2 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-xl z-50`}
        >
          ...
          <div className="max-h-64 overflow-y-auto scrollbar-thin py-1">
            {models.length > 0 ? (
              models.map((model) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {model.size != null && (
                      <span className="text-[10px] text-[var(--text-muted)]">{formatModelSize(model.size)}</span>
                    )}
                  </div>
                  {currentModel === model.name && <Check className="w-4 h-4 text-emerald-400 flex-shrink-0" />}
                </button>
              ))
            ) : (
              <div className="px-4 py-6 text-center">
                ...
                <p className="text-xs text-[var(--text-muted)] opacity-70 mt-1">Check GPU connection</p>
              </div>
            )}
          </div>
          {models.length > 0 && (
